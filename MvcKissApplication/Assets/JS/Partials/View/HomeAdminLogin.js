(function() {
  window.app.controller('HomeAdminLoginController', [
    '$scope', '$rootScope', '$state', '$timeout', 'AdministratorService', 'AlertService', function($scope, $rootScope, $state, $timeout, AdministratorService, AlertService) {
      $scope.admin = {
        email: '',
        password: ''
      };
      return $scope.tryLogin = function() {
        return AdministratorService.getAdministrator($scope.admin.email, $scope.admin.password).success(function(data, status, headers, config) {
