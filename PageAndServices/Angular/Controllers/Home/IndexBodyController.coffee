...
['$scope', '$state', 'AlertService', 'CategoryService', 'TestimonialService', 'TextService', 'ValidationService',
( $scope,   $state,   AlertService,   CategoryService,   TestimonialService,   TextService,   ValidationService) ->
    ...
    $scope.goToBooking = (postalCode) ->

        if ValidationService.stockholmPostalCode postalCode
            $state.go 'book', { serviceName: '' }
        else
            AlertService.addAlert 'danger', 'Postnumret är inte inom Stockholms län.'
