(function() {
  window.app.controller('HomeIndexBodyController', [
    '$scope', '$state', 'AlertService', 'CategoryService', 'TestimonialService', 'TextService', 'ValidationService', function($scope, $state, AlertService, CategoryService, TestimonialService, TextService, ValidationService) {
      $scope.isAdmin = false;
      ...
      $scope.goToBooking = function(postalCode) {
        if (ValidationService.stockholmPostalCode(postalCode)) {
          return $state.go('book', {
            serviceName: ''
          });
        } else {
          return AlertService.addAlert('danger', 'Postnumret är inte inom Stockholms län.');
        }
      };
